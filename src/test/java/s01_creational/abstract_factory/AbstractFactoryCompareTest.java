...
public class AbstractFactoryCompareTest {
    ...
    class 정리 {
        ...
        void 비교_요약() {
             ...
             * ├─────────────────┼────────────────────────┼────────────────────────┤
             * │ 생성 대상         │ 단일 객체                │ 관련 객체 군              │
             * │ 구조             │ 상속 기반 (추상 클래스)     │ 조합 기반 (인터페이스)      │
             * │ 확장             │ 새 Creator 추가          │ 새 Factory 추가         │
             * │ 제품 타입 추가     │ Creator당 1개 수정        │ 모든 Factory 수정        │
             * │ 일관성 보장        │ X                      │ O (제품군 일관성)         │
             * │ Template 결합    │ 자연스러움                │ 별도 구현 필요            │
             * └─────────────────┴────────────────────────┴────────────────────────┘
             *
             * 선택 기준:
             * - 단일 객체 + 생성 후 로직 → Factory Method
             * - 여러 객체 + 일관성 필요 → Abstract Factory
             */
        }
    }
}
